...
TEST(Viewer, ViewerTest) {
    ...
    using PlyLoadResult = decltype(fvdb::detail::io::loadGaussianPly(""));
    std::vector<PlyLoadResult> loadedData;
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (size_t i = 0; i < std::size(ply_paths) && i < std::size(view_names); ++i) {
            ...
            fvdb::detail::viewer::GaussianSplat3dView &view = viewer.addGaussianSplat3dView(
                "default", view_name, means, quats, logScales, logitOpacities, sh0, shN);
            ...
            fvdb::detail::viewer::CameraView &cameraView = viewer.addCameraView("default",
                                                                                view_name,
                                                                                cameraToWorld,
                                                                                projectionMat,
                                                                                imageSizes,
    ...
    fvdb::detail::viewer::GaussianSplat3dView &view = viewer.addGaussianSplat3dView(
        "default", "test_view", means, quats, logScales, logitOpacities, sh0, shN);
