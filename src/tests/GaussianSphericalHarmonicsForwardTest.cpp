...
struct SphericalHarmonicsForwardTestFixture : public ::testing::TestWithParam<TestParams> {
    ...
    SetUp() override {
        ...
        if (setZeroRadii) {
            ...
            expectedResult.index_put_({torch::indexing::Slice(),
...
TEST_P(SphericalHarmonicsForwardTestFixture, TestShForward) {
    if (shDegreeToUse == 0) {
        {
            auto result = fvdb::detail::ops::evaluateSphericalHarmonicsFwd(shDegreeToUse,
                                                                           numCameras,
                                                                           means,
                                                                           worldToCamMatrices,
                                                                           cameraIds,
...
INSTANTIATE_TEST_SUITE_P(ShForwardTests,
                         SphericalHarmonicsForwardTestFixture,
