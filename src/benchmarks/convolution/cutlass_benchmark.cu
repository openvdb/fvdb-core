...
#define CUTLASS_TOPO_DENSE_SIZES                                                       \
    ->Args({10})->Args({20})->Args({40})->Args({60})->Args({80})->UseRealTime()->Unit( \
        benchmark::kMillisecond)

#define CUTLASS_TOPO_SPHERE_SIZES \
    ->Args({8})->Args({16})->Args({32})->Args({50})->UseRealTime()->Unit(benchmark::kMillisecond)
...
BM_Forward(benchmark::State &state, Backend backend, GridShape shape) {
    ...
    if (backend == Backend::GroupedGemm) {
        ...
        auto features_f16 = torch::randn({N, C_in}, torch::dtype(torch::kFloat16).device(device));
        auto weights_f16 =
...
BM_Backward(benchmark::State &state, Backend backend, GridShape shape) {
    ...
    if (backend == Backend::GroupedGemm) {
        ...
        auto weights =
            torch::randn({C_out, C_in, 3, 3, 3}, torch::dtype(torch::kFloat32).device(device));
        auto grad_output = torch::randn({N, C_out}, torch::dtype(torch::kFloat32).device(device));
        auto topo        = ops::groupedGemmSparseConvTopology(*grid, *grid, kernel_size, stride);
        ...
        // Warmup
        auto [gf, gw] = ops::cutlassConvBackward(grad_output_f16, features_f16, weights_f16, topo);
        torch::cuda::synchronize();
        ...
        // Verify grad_features against fp32 reference
        {
            ...
            auto gg_topo = ops::groupedGemmSparseConvTopology(*grid, *grid, kernel_size, stride);
            auto [ref_gf, ref_gw] = ops::groupedGemmSparseConvBackward(
                grad_output_f32, features_f32, weights_f32, gg_topo);
            torch::cuda::synchronize();
