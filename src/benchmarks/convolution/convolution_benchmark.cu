...
//   1. GatherScatterDefault: gatherScatterDefaultSparseConv (dense kernel map, per-offset GEMM)
//   2. GatherScatterFused:   gatherScatterSparseConvFused   (single-kernel, no intermediates)
//   3. GroupedGemm:          groupedGemmSparseConv           (compacted CSR, per-offset GEMM)
//
...
enum class ConvMethod { GatherScatterDefault, GatherScatterFused, GroupedGemm };
enum class GridShape { Dense, Sphere };

[[maybe_unused]] static char const *
methodStr(ConvMethod m) {
    switch (m) {
    case ConvMethod::GatherScatterDefault: return "GatherScatterDefault";
...
// =============================================================================
//
// Each benchmark computes a reference output using the GatherScatterDefault GEMM
...
// Compute the GatherScatterDefault (GEMM) reference output for a same-topology,
...
// =============================================================================
//
// Templated on device type so that CUDA syncs resolve at compile time.
// Each ConvMethod branch handles its own topology construction, warmup,
// verification, and timed loop.
...
BM_Conv_Impl(benchmark::State &state, ConvMethod method, GridShape shape) {
    ...
    // ----- GatherScatterDefault (dense kernel-map GEMM) -----
    if (method == ConvMethod::GatherScatterDefault) {
        auto topo = ops::gatherScatterDefaultSparseConvTopology(*grid, *grid, kernel_size, stride);
        ...
        if constexpr (isCuda) {
            ...
            torch::cuda::synchronize();
            if (!verifyAgainstReference(state, out, fused_ref, "GatherScatterDefault"))
                return;
        }

        for (auto _: state) {
            auto result = ops::gatherScatterDefaultSparseConv(features, weights, topo);
            if constexpr (isCuda) {
                torch::cuda::synchronize();
            }
...
BM_Topology_Impl(benchmark::State &state, ConvMethod method, GridShape shape) {
    ...
    if (method == ConvMethod::GatherScatterDefault) {
        ...
        for (auto _: state) {
            ...
            benchmark::DoNotOptimize(topo.gatherIndices.data_ptr<int32_t>());
        }
...
DEF_CONV_CUDA(Dense,  GatherScatterDefault, GatherScatterDefault, Dense)
DEF_CONV_CUDA(Dense,  GatherScatterFused,   GatherScatterFused,   Dense)
DEF_CONV_CUDA(Dense,  GroupedGemm,          GroupedGemm,          Dense)

DEF_CONV_CUDA(Sphere, GatherScatterDefault, GatherScatterDefault, Sphere)
...
DEF_CONV_CPU(Dense,  GatherScatterDefault, GatherScatterDefault, Dense)
DEF_CONV_CPU(Dense,  GatherScatterFused,   GatherScatterFused,   Dense)
DEF_CONV_CPU(Dense,  GroupedGemm,          GroupedGemm,          Dense)

DEF_CONV_CPU(Sphere, GatherScatterDefault, GatherScatterDefault, Sphere)
...
DEF_TOPO_CUDA(Dense,  GatherScatterDefault, GatherScatterDefault, Dense)
DEF_TOPO_CUDA(Dense,  GroupedGemm,          GroupedGemm,          Dense)

DEF_TOPO_CUDA(Sphere, GatherScatterDefault, GatherScatterDefault, Sphere)
...
BENCHMARK(BM_Conv_Dense_GatherScatterDefault) CONV_BENCH_SIZES;
...
BENCHMARK(BM_Conv_Sphere_GatherScatterDefault) SPHERE_BENCH_SIZES;
...
BENCHMARK(BM_Conv_Dense_GatherScatterDefault_CPU) CPU_CONV_BENCH_SIZES;
...
BENCHMARK(BM_Conv_Sphere_GatherScatterDefault_CPU) CPU_SPHERE_BENCH_SIZES;
...
BENCHMARK(BM_Topology_Dense_GatherScatterDefault) DENSE_TOPO_SIZES;
BENCHMARK(BM_Topology_Dense_GroupedGemm) DENSE_TOPO_SIZES;

BENCHMARK(BM_Topology_Sphere_GatherScatterDefault) SPHERE_TOPO_SIZES;
