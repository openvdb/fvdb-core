...
#     3. Speedup-vs-baseline bar chart (GatherScatterFused, GroupedGemm vs GatherScatter)
#   CUDA only:
#     4. Speedup heatmap (each backend vs GatherScatter baseline)
...
_BACKEND_ORDER = ["GatherScatter", "GatherScatterFused", "GroupedGemm"]
...
# Baseline backend for speedup comparisons
_BASELINE_BACKEND = "GatherScatter"

# Backends to compare head-to-head against the baseline
_COMPARE_BACKENDS = ["GatherScatterFused", "GroupedGemm"]
...
_KNOWN_BACKENDS = sorted(
    ["GatherScatter", "GatherScatterFused", "GroupedGemm"],
    key=len,
    reverse=True,
)
...
# ---------------------------------------------------------------------------
# Plot: Speedup-vs-baseline bar chart (focused comparison)
# ---------------------------------------------------------------------------


def plot_speedup_bars(df, device, suffix):
    """Bar chart: speedup of each backend over the baseline, per grid size."""
    conv = df[(df["Category"] == "Convolution") & (df["Device"] == device)]
    if conv.empty:
        return
        ...
        for ax, dim in zip(axes[0], dims):
            ...
            for i, backend in enumerate(compare):
                ...
                for ch in channels:
                    row = bd[bd["C_in"] == ch]
                    key = (dim, ch)
                    if not row.empty and key in baseline_idx.index and row.iloc[0]["Time_ms"] > 0:
                        speedups.append(baseline_idx[key] / row.iloc[0]["Time_ms"])
                    else:
                        speedups.append(0.0)
            ...
            ax.axhline(y=1.0, color=_color(_BASELINE_BACKEND), linestyle="--", linewidth=1.5, alpha=0.7)
            ax.set_xticks(x)
            ax.set_xticklabels([f"C={c}" for c in channels])
            ax.set_ylabel(f"Speedup over {baseline_label}")
            ax.set_title(f"{_grid_label(grid, dim)}", fontsize=11)
            ax.legend(fontsize=7, loc="upper left")
            ax.grid(axis="y", alpha=0.3)

        baseline_label = _label(_BASELINE_BACKEND)
        fig.suptitle(f"{grid} Grid -- Speedup over {baseline_label} [{device}]", fontsize=13, y=1.02)
        fig.tight_layout()
        _save(fig, f"conv_speedup_bars_{grid.lower()}_{suffix}.png")


# ---------------------------------------------------------------------------
# Plot: Speedup heatmap (CUDA only, each backend vs baseline)
# ---------------------------------------------------------------------------


def plot_speedup_heatmap(df):
    """Heatmap of speedup over baseline for every (grid_size, channel) pair."""
    ...
    for grid in sorted(conv["Grid"].unique()):
        ...
        baseline_label = _label(_BASELINE_BACKEND)
        for ax, nb in zip(axes[0], compare):
        ...
        fig.suptitle(f"{grid} Grid -- Speedup over {baseline_label} [CUDA]", fontsize=13, y=1.02)
        fig.tight_layout()
...
def _print_speedup_table(gdata, grid, baseline_idx, backends):
    """Print a per-backend speedup table relative to the baseline."""
    for backend in backends:
        ...
        for _, row in bd.sort_values(["Dim", "C_in"]).iterrows():
            key = (row["Dim"], row["C_in"])
            if key in baseline_idx.index and row["Time_ms"] > 0:
                su = baseline_idx[key] / row["Time_ms"]
                speedups.append(su)
