...
template <typename T>
inline void heavy_compute_kernel(const T* in, T* out, int64_t size) {
...
template <typename T>
inline void unbalanced_kernel(const T* in, T* out, int64_t size, int64_t offset) {
    for (int64_t i = 0; i < size; ++i) {
...
static void BM_Uniform_Serial(benchmark::State& state) {
    SetupThreads();
...
static void BM_Uniform_OpenMP(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Uniform_BasicPool(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Uniform_WorkStealing(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Uniform_ATParallel(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
void adaptive_parallel_for(int64_t start, int64_t end, Func&& f) {
    ...
    if (numel <= kSerialThreshold) {
        ...
        // WorkStealing for medium workloads
        dispatch::work_stealing_pool::instance().parallel_for(start, end, std::forward<Func>(f));
    }
}
}  // namespace

static void BM_Uniform_Adaptive(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_Serial(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_OpenMP_Static(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_OpenMP_Dynamic(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_BasicPool(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_WorkStealing(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_Adaptive(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
...
static void BM_Unbalanced_ATParallel(benchmark::State& state) {
    SetupThreads();
    const int64_t numel = state.range(0);
    auto input = torch::randn({numel});
    auto output = torch::empty_like(input);
