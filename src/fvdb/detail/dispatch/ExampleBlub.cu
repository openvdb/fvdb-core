...
// is compiled only by the host compiler (not nvcc). NVCC doesn't support
...
constexpr auto blub_impl_factory =
    []<torch::DeviceType DeviceValue, torch::ScalarType InStype, torch::ScalarType OutStype>()
    -> BlubFunctionPtr {
    return [](torch::Tensor in, torch::ScalarType out_dtype) -> torch::Tensor {
        ...
        auto concrete_in  = ::fvdb::dispatch::ConcreteTensor<DeviceValue, InStype, 1>{in};
        auto concrete_out = ::fvdb::dispatch::ConcreteTensor<DeviceValue, OutStype, 1>{out};
