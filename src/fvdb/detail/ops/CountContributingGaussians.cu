...
dispatch_count_contributing_gaussians(const torch::Tensor &means2d,
...
template <torch::DeviceType>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_count_contributing_gaussians_sparse(const torch::Tensor &means2d,
                                             const torch::Tensor &conics,
                                             const torch::Tensor &opacities,
                                             const torch::Tensor &tileOffsets,
                                             const torch::Tensor &tileGaussianIds,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_count_contributing_gaussians<torch::kCUDA>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_count_contributing_gaussians<torch::kCPU>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_count_contributing_gaussians_sparse<torch::kCUDA>(
    const torch::Tensor &means2d,     // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,      // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,   // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_count_contributing_gaussians_sparse<torch::kCPU>(
    const torch::Tensor &means2d,     // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,      // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,   // [C, N] or [nnz]
    const torch::Tensor &tileOffsets, // [C, tile_height, tile_width] (dense) or [AT + 1] (sparse)
...
count_contributing_gaussians(const torch::Tensor &means2d,
    ...
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatch_count_contributing_gaussians<DeviceTag>(means2d,
                                                                conics,
                                                                opacities,
                                                                tileOffsets,
                                                                tileGaussianIds,
...
count_contributing_gaussians_sparse(const torch::Tensor &means2d,
                                    ...
                                    uint32_t tileSize) {
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatch_count_contributing_gaussians_sparse<DeviceTag>(means2d,
                                                                       conics,
                                                                       opacities,
                                                                       tileOffsets,
                                                                       tileGaussianIds,
