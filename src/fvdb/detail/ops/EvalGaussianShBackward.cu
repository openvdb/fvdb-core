...
dispatch_eval_gaussian_sh_bwd(const int64_t shDegreeToUse,
...
template <>
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
dispatch_eval_gaussian_sh_bwd<torch::kCUDA>(
    const int64_t shDegreeToUse,
    const int64_t numCameras,
    const int64_t numGaussians,
...
template <>
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
dispatch_eval_gaussian_sh_bwd<torch::kPrivateUse1>(
    const int64_t shDegreeToUse,
    const int64_t numCameras,
    const int64_t numGaussians,
    const torch::Tensor &viewDirs,               // [C, N, 3]
    ...
    if (hasShNCoeffs && K > 1) {
        ...
        if (computeDLossDViewDirs) {
            for (const auto deviceId: c10::irange(c10::cuda::device_count())) {
                ...
                std::vector<size_t> prefetchLocationIndices(prefetchPtrs.size(), 0);
...
template <>
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
dispatch_eval_gaussian_sh_bwd<torch::kCPU>(const int64_t shDegreeToUse,
                                           const int64_t numCameras,
                                           const int64_t numGaussians,
                                           const torch::Tensor &viewDirs,  // [C, N, 3] or empty
                                           const torch::Tensor &shNCoeffs, // [N, K-1, D]
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
eval_gaussian_sh_bwd(const int64_t shDegreeToUse,
                     const int64_t numCameras,
                     const int64_t numGaussians,
                     const torch::Tensor &viewDirs,  // [C, N, 3] or empty
                     const torch::Tensor &shNCoeffs, // [N, K-1, D]
    ...
    return FVDB_DISPATCH_KERNEL(dLossDColors.device(), [&]() {
        return dispatch_eval_gaussian_sh_bwd<DeviceTag>(shDegreeToUse,
                                                        numCameras,
                                                        numGaussians,
                                                        viewDirs,
                                                        shNCoeffs,
