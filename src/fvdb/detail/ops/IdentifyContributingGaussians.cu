...
launchRasterizeContributingGaussianIdsForwardKernel(
    ...
    if (numDepthSamples > 0) {
        ...
        if (pixelsToRender.has_value()) {
            ...
            std::tie(outIds, outWeights) =
                identify_top_contributing_gaussians_sparse(means2d,
                                                           conics,
                                                           opacities,
                                                           tileOffsets,
                                                           tileGaussianIds,
            ...
            std::tie(denseIds, denseWeights) = identify_top_contributing_gaussians(means2d,
                                                                                   conics,
                                                                                   opacities,
                                                                                   tileOffsets,
                                                                                   tileGaussianIds,
    ...
    const auto tileExtentH = (imageHeight + tileSize - 1) / tileSize;
...
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor> dispatch_identify_contributing_gaussians(
...
template <torch::DeviceType>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor> dispatch_identify_contributing_gaussians_sparse(
    const torch::Tensor &means2d,
    const torch::Tensor &conics,
    const torch::Tensor &opacities,
    const torch::Tensor &tile_offsets,
...
dispatch_identify_contributing_gaussians<torch::kCUDA>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_identify_contributing_gaussians<torch::kCPU>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
__host__ std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_identify_contributing_gaussians_sparse<torch::kCUDA>(
    const torch::Tensor &means2d,     // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,      // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,   // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_identify_contributing_gaussians_sparse<torch::kCPU>(
    const torch::Tensor &means2d,     // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,      // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,   // [C, N] or [nnz]
    const torch::Tensor &tileOffsets, // [C, tile_height, tile_width] (dense) or [AT + 1] (sparse)
...
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
identify_contributing_gaussians(const torch::Tensor &means2d,
                                const torch::Tensor &conics,
                                const torch::Tensor &opacities,
                                const torch::Tensor &tile_offsets,
                                const torch::Tensor &tile_gaussian_ids,
    ...
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatch_identify_contributing_gaussians<DeviceTag>(means2d,
                                                                   conics,
                                                                   opacities,
                                                                   tile_offsets,
                                                                   tile_gaussian_ids,
...
identify_contributing_gaussians_sparse(
    ...
    const std::optional<fvdb::JaggedTensor> &maybeNumContributingGaussians) {
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatch_identify_contributing_gaussians_sparse<DeviceTag>(
            means2d,
            conics,
            opacities,
            tile_offsets,
