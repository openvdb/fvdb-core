...
conv_backward_cuda_setting1_mode1_bf16bf16f32(int M_fwd,
                                              int K_original,
                                              int N,
                                              int kernel_volume,
                                              int split_k_iters,
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
conv_backward_cuda_setting1_mode1_f16f16f32(int M_fwd,
                                            int K_original,
                                            int N,
                                            int kernel_volume,
                                            int split_k_iters,
...
conv_backward_cuda_setting2_mode1_bf16bf16f32(int M_fwd,
                                              int K_original,
                                              int N,
                                              int kernel_volume,
                                              int split_k_iters,
...
__global__ void __launch_bounds__(64)
conv_backward_cuda_setting2_mode1_f16f16f32(int M_fwd,
                                            int K_original,
                                            int N,
                                            int kernel_volume,
                                            int split_k_iters,
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
conv_backward_cuda_setting1_mode1_tf32tf32f32(int M_fwd,
                                              int K_original,
                                              int N,
                                              int kernel_volume,
                                              int split_k_iters,
    ...
    for (int _i2_0_0 = 0; _i2_0_0 < K_iters - 1; ++_i2_0_0) {
        ...
        for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 4; ++ax0_ax1_fused_0_1) {
            ...
            global_load<N_ld_factor>(
                B_loaded[1], B_ptr + v0 * N + 4, B_pred_guard >> (4 * 4 / N_ld_factor));
            *(ulonglong4 *)(B_shared +
                            (((ax0_ax1_fused_0_1 * 640) + ((((int)threadIdx.x) >> 1) * 40)) +
                             ((((int)threadIdx.x) & 1) * 8))) =
...
__global__ void __launch_bounds__(64)
conv_backward_cuda_setting2_mode1_tf32tf32f32(int M_fwd,
                                              int K_original,
                                              int N,
                                              int kernel_volume,
                                              int split_k_iters,
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
conv_backward_cuda_setting1_mode1_f32f32f32(int M_fwd,
                                            int K_original,
                                            int N,
                                            int kernel_volume,
                                            int split_k_iters,
...
__global__ void __launch_bounds__(64)
conv_backward_cuda_setting2_mode1_f32f32f32(int M_fwd,
                                            int K_original,
                                            int N,
                                            int kernel_volume,
                                            int split_k_iters,
...
template <>
torch::Tensor
dispatchSparseConvolutionImplicitGEMMGradSorted<torch::kCUDA>(torch::Tensor _in_feats,
                                                              torch::Tensor _kernel,
                                                              torch::Tensor _out_in_map,
                                                              torch::Tensor _reduced_mask,
                                                              torch::Tensor _reorder_loc,
                                                              const int split_k_iters,
                                                              bool allow_tf32,
                                                              bool allow_fp16) {
            ...
            if (num_in_channels % 16 == 0) {
                if (num_out_channels % 16 == 0) {
                    conv_backward_cuda_setting1_mode1_f16f16f32<16, 16, false, false>
                        <<<num_blocks, threads_per_block>>>(_kernel.size(0),
                                                            num_in_channels,
                                                            num_out_channels,
                                                            kernel_volume,
                                                            split_k_iters,
...
template <>
torch::Tensor
dispatchSparseConvolutionImplicitGEMMGradSorted<torch::kCPU>(torch::Tensor _in_feats,
                                                             torch::Tensor _kernel,
                                                             torch::Tensor _out_in_map,
                                                             torch::Tensor _reduced_mask,
                                                             torch::Tensor _reorder_loc,
