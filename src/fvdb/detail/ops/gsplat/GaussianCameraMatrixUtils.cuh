...
loadWorldToCamRtRowMajor4x4(const T *m44, nanovdb::math::Mat3<T> &R, nanovdb::math::Vec3<T> &t) {
    // Row-major 4x4 with last column = translation.
    R = nanovdb::math::Mat3<T>(m44[0],
                               m44[1],
                               m44[2],   // 1st row
                               m44[4],
                               m44[5],
                               ...
                               m44[9],
                               m44[10]); // 3rd row
    t = nanovdb::math::Vec3<T>(m44[3], m44[7], m44[11]);
}

template <typename T, typename Acc44>
inline __device__ void
loadWorldToCamRtFromAccessor44(const Acc44 &m44 /* [C,4,4] */,
                               const int64_t camId,
                               nanovdb::math::Mat3<T> &R,
                               nanovdb::math::Vec3<T> &t) {
    R = nanovdb::math::Mat3<T>(m44[camId][0][0],
                               m44[camId][0][1],
    ...
    t = nanovdb::math::Vec3<T>(m44[camId][0][3], m44[camId][1][3], m44[camId][2][3]);
}
