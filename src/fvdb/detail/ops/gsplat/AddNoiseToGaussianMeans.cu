...
void dispatchAddNoiseToGaussianMeans(torch::Tensor &means,
...
template <typename ScalarType>
__global__ void
addNoiseToGaussianMeansKernel(int64_t localToGlobalOffset,
                              int64_t localSize,
...
launchGaussianMCMCAddNoise(torch::Tensor &means,                // [N, 3]
    ...
    addNoiseToGaussianMeansKernel<ScalarType><<<gridDim, blockDim, 0, stream>>>(
        offset,
        size,
...
template <>
void
dispatchAddNoiseToGaussianMeans<torch::kCUDA>(torch::Tensor &means,                // [N, 3]
...
template <>
void
dispatchAddNoiseToGaussianMeans<torch::kPrivateUse1>(torch::Tensor &means, // [N, 3] input/output
                                                     const torch::Tensor &logScales,      // [N, 3]
                                                     const torch::Tensor &logitOpacities, // [N]
                                                     const torch::Tensor &quats,          // [N, 4]
                                                     const float noiseScale,
...
template <>
void
dispatchAddNoiseToGaussianMeans<torch::kCPU>(torch::Tensor &means,           // [N, 3] input/output
                                             const torch::Tensor &logScales, // [N, 3]
                                             const torch::Tensor &logitOpacities, // [N]
                                             const torch::Tensor &quats,          // [N, 4]
                                             const float noiseScale,
...
void
addNoiseToGaussianMeans(torch::Tensor &means,                // [N, 3] input/output
                        const torch::Tensor &logScales,      // [N]
                        const torch::Tensor &logitOpacities, // [N]
                        const torch::Tensor &quats,          // [N, 4]
                        const float noiseScale,
    ...
    FVDB_DISPATCH_KERNEL(means.device(), [&]() {
        return dispatchAddNoiseToGaussianMeans<DeviceTag>(
