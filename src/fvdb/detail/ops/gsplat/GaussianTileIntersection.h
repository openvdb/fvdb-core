...
class DenseTileIntersections {
  public:
    ...
    DenseTileIntersections(const torch::Tensor &means2d,
                           ...
                           uint32_t imageWidth);
    DenseTileIntersections(torch::Tensor tileOffsets, torch::Tensor tileGaussianIds, uint32_t tileSize);

    const torch::Tensor &
    tileOffsets() const;

    const torch::Tensor &
    tileGaussianIds() const;

    uint32_t
    tileSize() const;

    int64_t
    totalIntersections() const;
    ...
    struct Accessor {
      private:
        Accessor(fvdb::TorchRAcc64<int32_t, 3> tileOffsetsIn,
            ...
            : mTileOffsets(tileOffsetsIn),
              mTileGaussianIds(tileGaussianIdsIn),
              mNumCameras(numCamerasIn),
              mRenderWidth(renderWindow.width),
              mRenderHeight(renderWindow.height),
              mTileOriginW(renderWindow.originW / tileSizeIn),
              mTileOriginH(renderWindow.originH / tileSizeIn),
              mTileSize(tileSizeIn),
              mBlockOffset(blockOffsetIn),
              mTotalIntersections(totalIntersectionsIn) {
        ...
        template <typename CallbackT>
        friend inline void dispatchTileIntersectionsAccessor(const torch::Tensor &tileOffsets,
                                                             const torch::Tensor &tileGaussianIds,
                                                             const RenderWindow2D &renderWindow,
                                                             uint32_t tileSize,
                                                             uint32_t blockOffset,
      ...
      public:
        ...
        __host__ __device__ inline uint32_t numTilesW() const { return mNumTilesW; }
        __host__ __device__ inline uint32_t numTilesH() const { return mNumTilesH; }
        __host__ __device__ inline uint32_t numCameras() const { return mNumCameras; }
        __host__ __device__ inline uint32_t cameraCount(uint32_t fallback) const {
            (void)fallback;
            return mNumCameras;
        }
        __host__ __device__ inline uint32_t tileSize() const { return mTileSize; }
        __host__ __device__ inline uint32_t numActiveTiles() const {
            return mNumCameras * mNumTilesW * mNumTilesH;
        }
        __host__ __device__ inline uint32_t pixelCount() const {
            return mNumCameras * mRenderWidth * mRenderHeight;
        }
        __host__ __device__ inline uint32_t outputNumTensors(uint32_t numCameras) const {
            (void)numCameras;
            return 1;
        }
        ...
        cameraTileId(uint32_t tileOrdinal,
            ...
            return {cameraId, localRow + static_cast<int32_t>(tileOriginH),
        ...
        tilePixelActiveFromBlock(uint32_t blockOrdinal,
                                 ...
                                 uint32_t row,
                                 uint32_t col) const {
            return tilePixelActive(
                mTileSize, globalTileOrdinal(blockOrdinal), iInTile, jInTile, row, col, mRenderWidth, mRenderHeight);
        }

        __device__ inline cuda::std::tuple<bool, uint32_t>
        activePixelIndexFromBlock(
            ...
            (void)scratch;
            const bool pixelActive = tilePixelActiveFromBlock(blockOrdinal, iInTile, jInTile, row, col);
    ...
    Accessor
    accessor(const RenderWindow2D &renderWindow, uint32_t blockOffset) const;
#endif
    ...
    struct Accessor {
      private:
        Accessor(fvdb::TorchRAcc64<int32_t, 1> sparseTileOffsetsIn,
                 ...
                 uint32_t blockOffsetIn,
                 int32_t totalIntersectionsIn)
            : mSparseTileOffsets(sparseTileOffsetsIn),
              mTileGaussianIds(tileGaussianIdsIn),
              mActiveTiles(activeTilesIn),
              mTilePixelMask(tilePixelMaskIn),
              mTilePixelCumsum(tilePixelCumsumIn),
              mPixelMap(pixelMapIn),
      ...
      public:
        ...
        __host__ __device__ inline uint32_t numActiveTiles() const { return mNumActiveTiles; }
        __host__ __device__ inline uint32_t numTilesW() const { return mNumTilesW; }
        __host__ __device__ inline uint32_t numTilesH() const { return mNumTilesH; }
        __host__ __device__ inline uint32_t numCameras() const { return 0; }
        __host__ __device__ inline uint32_t cameraCount(uint32_t fallback) const { return fallback; }
        __host__ __device__ inline uint32_t tileSize() const { return mTileSize; }
        __host__ __device__ inline uint32_t pixelCount() const { return mPixelMap.size(0); }
        __host__ __device__ inline uint32_t outputNumTensors(uint32_t numCameras) const {
            return numCameras;
        }
        __host__ __device__ inline uint64_t
      ...
      private:
        fvdb::TorchRAcc64<int32_t, 1> mSparseTileOffsets; // [AT + 1]
        fvdb::TorchRAcc64<int32_t, 1> mTileGaussianIds;   // [totalIntersections]
...
dispatchTileIntersectionsAccessor(
    ...
    if (tileOffsets.dim() == 1) {
        TORCH_CHECK_VALUE(activeTiles.has_value(),
                          "activeTiles must be provided for sparse tile intersections");
