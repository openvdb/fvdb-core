...
dispatchIdentifyTopContributingGaussians(const torch::Tensor &means2d,
...
template <torch::DeviceType>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatchIdentifyTopContributingGaussiansSparse(const torch::Tensor &means2d,
                                               const torch::Tensor &conics,
                                               const torch::Tensor &opacities,
                                               const torch::Tensor &tile_offsets,
                                               const torch::Tensor &tile_gaussian_ids,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatchIdentifyTopContributingGaussians<torch::kCUDA>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatchIdentifyTopContributingGaussians<torch::kCPU>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatchIdentifyTopContributingGaussiansSparse<torch::kCUDA>(
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
    const torch::Tensor &tileOffsets,     // [C, tile_height, tile_width]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatchIdentifyTopContributingGaussiansSparse<torch::kCPU>(
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
    const torch::Tensor &tileOffsets,     // [C, tile_height, tile_width]
...
identifyTopContributingGaussians(const torch::Tensor &means2d,
    ...
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatchIdentifyTopContributingGaussians<DeviceTag>(means2d,
                                                                   conics,
                                                                   opacities,
                                                                   tile_offsets,
                                                                   tile_gaussian_ids,
...
identifyTopContributingGaussiansSparse(const torch::Tensor &means2d,
                                       ...
                                       int numDepthSamples) {
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatchIdentifyTopContributingGaussiansSparse<DeviceTag>(means2d,
                                                                         conics,
                                                                         opacities,
                                                                         tile_offsets,
                                                                         tile_gaussian_ids,
