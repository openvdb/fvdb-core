...
template <typename ScalarType, uint32_t NUM_CHANNELS, bool IS_PACKED, typename TileIntersectionsT>
...
rasterizeGaussiansForward(
    RasterizeForwardArgs<ScalarType, NUM_CHANNELS, IS_PACKED, TileIntersectionsT> args) {
...
launchRasterizeForwardKernel(
    ...
    const auto launchWithTileIntersections = [&](auto tileIntersections) {
        ...
        auto args = RasterizeForwardArgs<ScalarType, NUM_CHANNELS, IS_PACKED, TileIntersectionsT>(
            means2d,
            conics,
            opacities,
            features,
        ...
        if (cudaFuncSetAttribute(
                rasterizeGaussiansForward<ScalarType, NUM_CHANNELS, IS_PACKED, TileIntersectionsT>,
...
launchRasterizeForwardKernels(
    ...
    for (const auto deviceId: c10::irange(c10::cuda::device_count())) {
        ...
        std::tie(deviceTileOffset, deviceTileCount) = deviceChunk(tileCount, deviceId);
        if (deviceTileCount) {
            auto args = RasterizeForwardArgs<ScalarType,
                                             NUM_CHANNELS,
                                             IS_PACKED,
