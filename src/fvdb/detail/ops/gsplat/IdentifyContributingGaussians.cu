    ...
    if (numDepthSamples > 0) {
        ...
        if (pixelsToRender.has_value()) {
            ...
            std::tie(outIds, outWeights) =
                identifyTopContributingGaussiansSparse(means2d,
                                                       conics,
                                                       opacities,
                                                       tileOffsets,
                                                       tileGaussianIds,
            ...
            std::tie(denseIds, denseWeights) = identifyTopContributingGaussians(means2d,
                                                                                conics,
                                                                                opacities,
                                                                                tileOffsets,
                                                                                tileGaussianIds,
...
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor> dispatchIdentifyContributingGaussians(
...
template <torch::DeviceType>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor> dispatchIdentifyContributingGaussiansSparse(
    const torch::Tensor &means2d,
    const torch::Tensor &conics,
    const torch::Tensor &opacities,
    const torch::Tensor &tile_offsets,
...
dispatchIdentifyContributingGaussians<torch::kCUDA>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatchIdentifyContributingGaussians<torch::kCPU>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
__host__ std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatchIdentifyContributingGaussiansSparse<torch::kCUDA>(
    const torch::Tensor &means2d,     // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,      // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,   // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatchIdentifyContributingGaussiansSparse<torch::kCPU>(
    const torch::Tensor &means2d,     // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,      // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,   // [C, N] or [nnz]
    const torch::Tensor &tileOffsets, // [C, tile_height, tile_width] (dense) or [AT + 1] (sparse)
...
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
identifyContributingGaussians(const torch::Tensor &means2d,
                              const torch::Tensor &conics,
                              const torch::Tensor &opacities,
                              const torch::Tensor &tile_offsets,
                              const torch::Tensor &tile_gaussian_ids,
    ...
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatchIdentifyContributingGaussians<DeviceTag>(means2d,
                                                                conics,
                                                                opacities,
                                                                tile_offsets,
                                                                tile_gaussian_ids,
...
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
identifyContributingGaussiansSparse(
    const torch::Tensor &means2d,
    const torch::Tensor &conics,
    const torch::Tensor &opacities,
    const torch::Tensor &tile_offsets,
    ...
    const std::optional<fvdb::JaggedTensor> &maybeNumContributingGaussians) {
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatchIdentifyContributingGaussiansSparse<DeviceTag>(
            means2d,
            conics,
            opacities,
            tile_offsets,
