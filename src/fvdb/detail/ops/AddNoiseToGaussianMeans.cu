...
void dispatch_add_noise_to_gaussian_means(torch::Tensor &means,
...
template <typename ScalarType>
__global__ void
add_noise_to_gaussian_means_kernel(int64_t localToGlobalOffset,
                                   int64_t localSize,
...
launchGaussianMCMCAddNoise(torch::Tensor &means,                // [N, 3]
    ...
    add_noise_to_gaussian_means_kernel<ScalarType><<<gridDim, blockDim, 0, stream>>>(
        offset,
        size,
...
template <>
void
dispatch_add_noise_to_gaussian_means<torch::kCUDA>(torch::Tensor &means,                // [N, 3]
...
template <>
void
dispatch_add_noise_to_gaussian_means<torch::kPrivateUse1>(
    torch::Tensor &means,                // [N, 3] input/output
    const torch::Tensor &logScales,      // [N, 3]
    const torch::Tensor &logitOpacities, // [N]
    const torch::Tensor &quats,          // [N, 4]
    const float noiseScale,
...
template <>
void
dispatch_add_noise_to_gaussian_means<torch::kCPU>(torch::Tensor &means, // [N, 3] input/output
                                                  const torch::Tensor &logScales,      // [N, 3]
                                                  const torch::Tensor &logitOpacities, // [N]
                                                  const torch::Tensor &quats,          // [N, 4]
                                                  const float noiseScale,
...
void
add_noise_to_gaussian_means(torch::Tensor &means,                // [N, 3] input/output
                            const torch::Tensor &logScales,      // [N]
                            const torch::Tensor &logitOpacities, // [N]
                            const torch::Tensor &quats,          // [N, 4]
                            const float noiseScale,
    ...
    FVDB_DISPATCH_KERNEL(means.device(), [&]() {
        return dispatch_add_noise_to_gaussian_means<DeviceTag>(
