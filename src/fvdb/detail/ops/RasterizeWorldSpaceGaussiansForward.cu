...
#include <fvdb/detail/utils/gaussian/GaussianRasterizeFromWorld.cuh>
#include <fvdb/detail/utils/gaussian/GaussianRasterizeOptionalInputs.h>
...
launchForward(const torch::Tensor &means,
    ...
    const PreparedRasterOptionalInputs opt = prepare_raster_optional_inputs(
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
rasterize_world_space_gaussians_fwd(const torch::Tensor &means,
                                    const torch::Tensor &quats,
                                    const torch::Tensor &logScales,
                                    const torch::Tensor &features,
                                    const torch::Tensor &opacities,
