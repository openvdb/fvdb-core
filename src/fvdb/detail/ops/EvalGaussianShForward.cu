...
torch::Tensor dispatch_eval_gaussian_sh_fwd(const int64_t shDegreeToUse,
...
template <>
torch::Tensor
dispatch_eval_gaussian_sh_fwd<torch::kCUDA>(const int64_t shDegreeToUse,
                                            const int64_t numCameras,
                                            const torch::Tensor &viewDirs,  // [C, N, 3]
...
template <>
torch::Tensor
dispatch_eval_gaussian_sh_fwd<torch::kPrivateUse1>(const int64_t shDegreeToUse,
                                                   const int64_t numCameras,
                                                   const torch::Tensor &viewDirs,  // [C, N, 3]
                                                   const torch::Tensor &sh0Coeffs, // [N, 1, D]
                                                   const torch::Tensor &shNCoeffs, // [N, K-1, D]
...
template <>
torch::Tensor
dispatch_eval_gaussian_sh_fwd<torch::kCPU>(const int64_t shDegreeToUse,
                                           const int64_t numCameras,
...
torch::Tensor
eval_spherical_harmonics(const torch::Tensor &means,
    ...
    if (actualShDegree == 0) {
        return FVDB_DISPATCH_KERNEL(sh0.device(), [&]() {
            return dispatch_eval_gaussian_sh_fwd<DeviceTag>(actualShDegree,
                                                            C,
                                                            torch::Tensor(),
                                                            sh0,
                                                            torch::Tensor(),
        ...
        const torch::Tensor viewDirs =
            ...
            camToWorldMatrices.index({torch::indexing::Slice(),
                                      ...
                                      3}); // [1, N, 3] - [C, 1, 3]
        return FVDB_DISPATCH_KERNEL(sh0.device(), [&]() {
            return dispatch_eval_gaussian_sh_fwd<DeviceTag>(
...
torch::Tensor
eval_gaussian_sh_fwd(const int64_t shDegreeToUse,
                     const int64_t numCameras,
                     const torch::Tensor &viewDirs,  // [C, N, 3]
                     const torch::Tensor &sh0Coeffs, // [1, N, D]
                     const torch::Tensor &shNCoeffs, // [N, K-1, D]
