...
#include <fvdb/detail/ops/IdentifyTopContributingGaussians.h>
#include <fvdb/detail/utils/AccessorHelpers.cuh>
#include <fvdb/detail/utils/Nvtx.h>
#include <fvdb/detail/utils/Utils.h>
#include <fvdb/detail/utils/cuda/OpType.cuh>
#include <fvdb/detail/utils/gaussian/Gaussian2D.cuh>
#include <fvdb/detail/utils/gaussian/GaussianRasterize.cuh>
...
launchRasterizeTopContributingGaussianIdsForwardKernel(
    ...
    TORCH_CHECK_VALUE(numDepthSamples > 0, "numDepthSamples must be greater than 0");
    if (IS_PACKED) {
    ...
    auto outIdsData =
        torch::empty({totalSize, numDepthSamples}, means2d.options().dtype(torch::kInt32));

    auto outWeightsData =
        torch::empty({totalSize, numDepthSamples},
    ...
    const uint32_t sharedMem =
        ...
        (sizeof(Gaussian2D<ScalarType>) +
         (sizeof(int32_t) + sizeof(ScalarType) + sizeof(uint32_t)) * numDepthSamples);
    ...
    auto args           = RasterizeTopContributingGaussianIdsArgs<ScalarType, IS_PACKED>(means2d,
                                                                               conics,
                                                                               opacities,
                                                                               backgrounds,
                                                                               masks,
...
dispatch_identify_top_contributing_gaussians(const torch::Tensor &means2d,
...
template <torch::DeviceType>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_identify_top_contributing_gaussians_sparse(const torch::Tensor &means2d,
                                                    const torch::Tensor &conics,
                                                    const torch::Tensor &opacities,
                                                    const torch::Tensor &tile_offsets,
                                                    const torch::Tensor &tile_gaussian_ids,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_identify_top_contributing_gaussians<torch::kCUDA>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_identify_top_contributing_gaussians<torch::kCPU>(
    // Gaussian parameters
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_identify_top_contributing_gaussians_sparse<torch::kCUDA>(
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
    const torch::Tensor &tileOffsets,     // [C, tile_height, tile_width]
...
template <>
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
dispatch_identify_top_contributing_gaussians_sparse<torch::kCPU>(
    const torch::Tensor &means2d,         // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,          // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,       // [C, N] or [nnz]
    const torch::Tensor &tileOffsets,     // [C, tile_height, tile_width]
...
std::tuple<torch::Tensor, torch::Tensor>
identify_top_contributing_gaussians(const torch::Tensor &means2d,
                                    const torch::Tensor &conics,
                                    const torch::Tensor &opacities,
                                    const torch::Tensor &tile_offsets,
                                    const torch::Tensor &tile_gaussian_ids,
    ...
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatch_identify_top_contributing_gaussians<DeviceTag>(means2d,
                                                                       conics,
                                                                       opacities,
                                                                       tile_offsets,
                                                                       tile_gaussian_ids,
...
std::tuple<fvdb::JaggedTensor, fvdb::JaggedTensor>
identify_top_contributing_gaussians_sparse(const torch::Tensor &means2d,
                                           const torch::Tensor &conics,
                                           const torch::Tensor &opacities,
                                           const torch::Tensor &tile_offsets,
                                           const torch::Tensor &tile_gaussian_ids,
                                           ...
                                           int numDepthSamples) {
    return FVDB_DISPATCH_KERNEL_DEVICE(means2d.device(), [&]() {
        return dispatch_identify_top_contributing_gaussians_sparse<DeviceTag>(means2d,
                                                                              conics,
                                                                              opacities,
                                                                              tile_offsets,
                                                                              tile_gaussian_ids,
