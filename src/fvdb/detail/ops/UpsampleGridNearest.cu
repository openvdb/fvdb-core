...
struct upsample_grid_nearest_op {
    ...
    op(Tag tg,
       ...
       nanovdb::Coord upsamplingFactor) {
        ...
        torch::Tensor outFineData = torch::zeros(spliceShape({numFineVoxels}, originalCoarseData),
                                                 torch::TensorOptions()
                                                     .dtype(originalCoarseData.dtype())
        ...
        auto fine_out = ::dispatch::tensor_out<dev, stype, 2, ::dispatch::contiguity::contiguous>(
            outFineCoalesced);
        ...
        ::fvdb::detail::dispatch::forEachActiveVoxel(
            ...
            [=] __hostdev__(Tag /*tg*/,
                            ...
                            int64_t fineIndex,
                            GridBatchImpl::Accessor /*fineAcc*/) {
                nanovdb::Coord const coarseIjk =
                    nanovdb::math::Vec3<scalar_t>(static_cast<scalar_t>(fineIjk[0]) /
                                                      static_cast<scalar_t>(upsamplingFactor[0]),
                                                  static_cast<scalar_t>(fineIjk[1]) /
                                                      static_cast<scalar_t>(upsamplingFactor[1]),
                                                  static_cast<scalar_t>(fineIjk[2]) /
                                                      static_cast<scalar_t>(upsamplingFactor[2]))
                        .floor();

                auto const coarseGridAcc = coarse_acc.grid(batchIdx)->getAccessor();
                if (coarseGridAcc.isActive(coarseIjk)) {
                    int64_t const ci = static_cast<int64_t>(coarseGridAcc.getValue(coarseIjk)) - 1 +
...
struct upsample_grid_nearest_backward_op {
    ...
    op(Tag tg,
       ...
       nanovdb::Coord upsamplingFactor) {
        ...
        auto out_grad = ::dispatch::tensor_out<dev, stype, 2, ::dispatch::contiguity::contiguous>(
            outGradInCoalesced);
        auto coarse_acc = ::fvdb::detail::dispatch::make_grid_accessor(tg, coarseGrid);
...
upsampleGridNearestBackward(GridBatchImpl const &fineGrid,
    ...
    static auto const table = ::dispatch::dispatch_table_from_op<upsample_grid_nearest_backward_op>(
