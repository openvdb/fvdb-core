...
// The output is always int64 (contiguous).
...
struct ijk_to_index_op {
    ...
    op(Tag tg, GridBatchImpl const &grid, JaggedTensor ijk, bool cumulative) {
        ...
        auto out   = torch::empty({n}, torch::TensorOptions().dtype(torch::kLong).device(device));
...
ijkToIndex(GridBatchImpl const &grid, JaggedTensor ijk, bool cumulative) {
    ...
    static auto const table = ::dispatch::dispatch_table_from_op<ijk_to_index_op>("ijkToIndex");
