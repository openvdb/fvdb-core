...
//  (sparseProjectGaussiansImpl) deduplicates before calling build_sparse_gaussian_tile_layout and
...
build_sparse_gaussian_tile_layout(const int32_t tileSideLength,
                                  const int32_t numTilesW,
                                  const int32_t numTilesH,
                                  const fvdb::JaggedTensor &pixelsToRender) {
    FVDB_FUNC_RANGE();
    TORCH_CHECK_NOT_IMPLEMENTED(pixelsToRender.device().is_cuda(),
                                "build_sparse_gaussian_tile_layout only implemented on the device");
