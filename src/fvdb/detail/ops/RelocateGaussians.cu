...
dispatch_relocate_gaussians(const torch::Tensor &logScales,
...
__global__ void
relocate_gaussians_kernel(int64_t localToGlobalOffset,
                          int64_t localSize,
...
launchGaussianRelocation(const torch::Tensor &logScales,      // [N, 3]
    ...
    relocate_gaussians_kernel<ScalarType><<<gridDim, blockDim, 0, stream>>>(
        offset,
        size,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_relocate_gaussians<torch::kCUDA>(const torch::Tensor &logScales,      // [N, 3]
                                          const torch::Tensor &logitOpacities, // [N]
                                          const torch::Tensor &ratios,         // [N]
                                          const torch::Tensor &binomialCoeffs, // [nMax, nMax]
                                          const int nMax,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_relocate_gaussians<torch::kPrivateUse1>(
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_relocate_gaussians<torch::kCPU>(const torch::Tensor &logScales,      // [N, 3]
                                         const torch::Tensor &logitOpacities, // [N]
                                         const torch::Tensor &ratios,         // [N]
                                         const torch::Tensor &binomialCoeffs, // [nMax, nMax]
                                         const int nMax,
...
relocate_gaussians(const torch::Tensor &logScales,      // [N, 3]
    ...
    return FVDB_DISPATCH_KERNEL(logScales.device(), [&]() {
        return dispatch_relocate_gaussians<DeviceTag>(
