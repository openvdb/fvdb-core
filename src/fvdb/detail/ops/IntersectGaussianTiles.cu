...
#include <fvdb/detail/utils/cuda/CubWrapper.cuh>
...
intersect_gaussian_tiles_cuda_impl(
...
std::tuple<torch::Tensor, torch::Tensor>
intersect_gaussian_tiles_private_use1_impl(
    const torch::Tensor &means2d,                   // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                     // [C, N] or [M]
    const torch::Tensor &depths,                    // [C, N] or [M]
    const at::optional<torch::Tensor> &cameraJIdx,  // NULL or [M]
...
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles(const torch::Tensor &means2d, // [C, N, 2] or [M, 2]
                                  const torch::Tensor &radii,   // [C, N] or [M]
                                  const torch::Tensor &depths,  // [C, N] or [M]
...
template <torch::DeviceType>
std::tuple<torch::Tensor, torch::Tensor> dispatch_intersect_gaussian_tiles_sparse(
    const torch::Tensor &means2d,                 // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                   // [C, N] or [M]
    const torch::Tensor &depths,                  // [C, N] or [M]
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles<torch::kCUDA>(
    const torch::Tensor &means2d,                  // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                    // [C, N] or [M]
    const torch::Tensor &depths,                   // [C, N] or [M]
    const at::optional<torch::Tensor> &cameraJIdx, // NULL or [M]
    ...
    const uint32_t numTilesW) {
    FVDB_FUNC_RANGE();
    return intersect_gaussian_tiles_cuda_impl(means2d,
                                              radii,
                                              depths,
                                              cameraJIdx,
                                              at::nullopt,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles<torch::kPrivateUse1>(
    const torch::Tensor &means2d,                  // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                    // [C, N] or [M]
    const torch::Tensor &depths,                   // [C, N] or [M]
    const at::optional<torch::Tensor> &cameraJIdx, // NULL or [M]
    ...
    const uint32_t numTilesW) {
    FVDB_FUNC_RANGE();
    return intersect_gaussian_tiles_private_use1_impl(means2d,
                                                      radii,
                                                      depths,
                                                      cameraJIdx,
                                                      at::nullopt,
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles<torch::kCPU>(
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles_sparse<torch::kCUDA>(
    const torch::Tensor &means2d,                  // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                    // [C, N] or [M]
    const torch::Tensor &depths,                   // [C, N] or [M]
    const torch::Tensor &tileMask,                 // [C, H, W]
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles_sparse<torch::kPrivateUse1>(
    const torch::Tensor &means2d,                  // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                    // [C, N] or [M]
    const torch::Tensor &depths,                   // [C, N] or [M]
    const torch::Tensor &tileMask,                 // [C, H, W]
...
template <>
std::tuple<torch::Tensor, torch::Tensor>
dispatch_intersect_gaussian_tiles_sparse<torch::kCPU>(
    const torch::Tensor &means2d,                  // [C, N, 2] or [M, 2]
    const torch::Tensor &radii,                    // [C, N] or [M]
    const torch::Tensor &depths,                   // [C, N] or [M]
    const torch::Tensor &tileMask,                 // [C, H, W]
...
std::tuple<torch::Tensor, torch::Tensor>
intersect_gaussian_tiles(const torch::Tensor &means2d,                 // [C, N, 2] or [M, 2]
                         const torch::Tensor &radii,                   // [C, N] or [M]
                         const torch::Tensor &depths,                  // [C, N] or [M]
                         const at::optional<torch::Tensor> &cameraIds, // NULL or [M]
                         const uint32_t numCameras,
    ...
    return FVDB_DISPATCH_KERNEL(means2d.device(), [&]() {
        return dispatch_intersect_gaussian_tiles<DeviceTag>(
            means2d, radii, depths, cameraIds, numCameras, tileSize, numTilesH, numTilesW);
    });
}

std::tuple<torch::Tensor, torch::Tensor>
intersect_gaussian_tiles_sparse(const torch::Tensor &means2d,                 // [C, N, 2] or [M, 2]
                                const torch::Tensor &radii,                   // [C, N] or [M]
                                const torch::Tensor &depths,                  // [C, N] or [M]
                                const torch::Tensor &tileMask,                // [C, H, W]
                                const torch::Tensor &activeTiles,             // [num_active_tiles]
                                ...
                                const uint32_t numTilesW) {
    return FVDB_DISPATCH_KERNEL(means2d.device(), [&]() {
        return dispatch_intersect_gaussian_tiles_sparse<DeviceTag>(means2d,
                                                                   radii,
                                                                   depths,
                                                                   tileMask,
                                                                   activeTiles,
