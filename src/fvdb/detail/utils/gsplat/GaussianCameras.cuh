...
//
#ifndef FVDB_DETAIL_UTILS_GSPLAT_GAUSSIANCAMERAS_CUH
#define FVDB_DETAIL_UTILS_GSPLAT_GAUSSIANCAMERAS_CUH

#include <fvdb/detail/utils/cuda/Alignment.cuh>
...
using fvdb::detail::alignUpAddress;
using fvdb::detail::alignUpBytes;
using fvdb::detail::clamp01;
using fvdb::detail::normalizeSafe;
...
/// @brief Returns true when an axis-aligned footprint box does not overlap the image.
///
/// This helper checks the AABB centered at `mean2d` with half-extents (`radiusX`, `radiusY`).
/// It does not perform exact ellipse clipping.
template <typename T>
...
template <typename T> struct PerspectiveCamera {
  public:
    ...
    PerspectiveCamera(const torch::Tensor &projectionMatrices,
        ...
        : projectionMatricesAcc(
          ...
          imageWidth(imageWidth), imageHeight(imageHeight), nearPlane(nearPlane),
          farPlane(farPlane) {}
  ...
  public:
    ...
    /// centered at `mean2d` with half-extents (`radiusX`, `radiusY`) in pixels. Returns:
    ...
    /// @brief Returns world-to-camera rotation and translation for camera `cid`.
    ///
    ...
    /// @brief Projects a camera-space 3D Gaussian to pixel-space `(cov2d, mean2d)`.
    ///
    /// Returns `(cov2d, mean2d)` where `cov2d` is the projected 2x2 covariance and `mean2d`
    ...
    /// @brief Backpropagates projection gradients to camera-space covariance and mean.
    ///
    ...
    /// @brief Projects a world-space Gaussian directly to 2D outputs.
    ///
    /// Internally applies world-to-camera transform and covariance transform before projection.
    /// Returns `(cov2d, mean2d, depthCam)`.
    inline __device__ std::tuple<nanovdb::math::Mat2<T>, nanovdb::math::Vec2<T>, T>
    ...
    ///
    /// Returns `(dLossDCovarWorld, dLossDMeanWorld, dLossDRotation, dLossDTranslation)`.
    inline __device__ std::tuple<nanovdb::math::Mat3<T>,
                                 nanovdb::math::Vec3<T>,
                                 nanovdb::math::Mat3<T>,
                                 nanovdb::math::Vec3<T>>
    projectWorldGaussianTo2DVJP(const int64_t cid,
                                const nanovdb::math::Vec3<T> &meansWorldSpace,
                                const nanovdb::math::Mat3<T> &covarWorldSpace,
                                const nanovdb::math::Mat2<T> &dLossDCovar2d,
        ...
        auto [dLossDRotation, dLossDTranslation, dLossDMeanWorld] =
            transformPointWorldToCamVectorJacobianProduct(
                R, t, meansWorldSpace, dLossDMeanCamSpace);
        auto [dLossDRotationCov, dLossDCovarWorld] =
            transformCovarianceWorldToCamVectorJacobianProduct(
                R, covarWorldSpace, dLossDCovarCamSpace);
        dLossDRotation += dLossDRotationCov;
        return {dLossDCovarWorld, dLossDMeanWorld, dLossDRotation, dLossDTranslation};
    }
};
...
template <typename T> struct PerspectiveWithDistortionCamera {
  ...
  private:
    ...
    inline __device__ void
    bindSharedMemory(const uint32_t cameraCount, void *sharedMemory) {
        ...
        addr += static_cast<size_t>(cameraCount) * sizeof(Mat3);
        addr                       = alignUpAddress(addr, alignof(Vec3));
        worldToCamStartTransShared = reinterpret_cast<Vec3 *>(addr);
        addr += static_cast<size_t>(cameraCount) * sizeof(Vec3);
        addr                   = alignUpAddress(addr, alignof(Mat3));
  ...
  public:
    ...
    loadSharedMemory(void *sharedMemory) {
        sharedCameraOffset = 0;
        bindSharedMemory(numCameras, sharedMemory);
    ...
    /// @brief Loads one camera's poses, intrinsics, and distortion into shared memory.
    inline __device__ void
    loadSharedMemory(const int64_t cid, void *sharedMemory) {
        sharedCameraOffset = cid;
        bindSharedMemory(1, sharedMemory);
        copyMat3Accessor<T>(1, worldToCamStartRotShared, worldToCamStartAcc, cid);
        copyWorldToCamTranslation<T>(1, worldToCamStartTransShared, worldToCamStartAcc, cid);
        copyMat3Accessor<T>(1, worldToCamEndRotShared, worldToCamEndAcc, cid);
        copyWorldToCamTranslation<T>(1, worldToCamEndTransShared, worldToCamEndAcc, cid);
        copyMat3Accessor<T>(1, projectionMatsShared, projectionMatricesAcc, cid);
        if (numDistCoeffs > 0) {
            copyDistortionCoeffs<T>(1, numDistCoeffs, distortionShared, distortionCoeffsAcc, cid);
        }
    }
    ...
    /// @brief Projects a world-space point and returns visibility classification.
    ///
    /// Writes the projected pixel to `outPixel` and returns:
  ...
  private:
    /// @brief Returns start/end world-to-camera transforms for camera `cid`.
    ...
    worldToCamRtStartEnd(const int64_t cid) const {
        if (worldToCamStartRotShared != nullptr) {
            const int64_t sharedCid = cid - sharedCameraOffset;
            return std::make_tuple(worldToCamStartRotShared[sharedCid],
                                   worldToCamStartTransShared[sharedCid],
                                   worldToCamEndRotShared[sharedCid],
                                   worldToCamEndTransShared[sharedCid]);
        }
    ...
    /// @brief Returns intrinsic matrix for camera `cid`.
    inline __device__ Mat3
    projectionMatrix(const int64_t cid) const {
        if (projectionMatsShared != nullptr) {
            return projectionMatsShared[cid - sharedCameraOffset];
        }
        const auto K = projectionMatricesAcc[cid];
        return Mat3(
    ...
    /// @brief Returns packed distortion coefficients for camera `cid`, or null.
    ...
    distortionPtr(const int64_t cid) const {
        ...
        if (distortionShared != nullptr) {
            return distortionShared + (cid - sharedCameraOffset) * numDistCoeffs;
        }
        return &distortionCoeffsAcc[cid][0];
    }

    /// @brief Converts pixel position to normalized rolling-shutter time in [0, 1].
...
template <typename T> struct OrthographicWithDistortionCamera {
  ...
  public:
    ...
    /// @brief Loads one camera's poses and intrinsics into shared memory.
    inline __device__ void
    loadSharedMemory(const int64_t cid, void *sharedMemory) {
        sharedCameraOffset = cid;
        bindSharedMemory(1, sharedMemory);
...
#endif // FVDB_DETAIL_UTILS_GSPLAT_GAUSSIANCAMERAS_CUH
