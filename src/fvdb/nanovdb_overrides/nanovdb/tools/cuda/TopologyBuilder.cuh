...
/*!
    ...
    \note FVDB FORK: This header is copied from upstream nanoVDB. The only
          functional change vs upstream is an env-gated debug print in
          `allocateInternalMaskBuffers` that reports the tile count and
          per-call scratch size, used to diagnose topology-operator
          memory blowup when dilating / merging large scenes. Everything
          else is byte-identical with upstream; keep the banner and
          matching `FVDB FORK:` tag on any non-trivial future delta so
          resync stays easy. See src/fvdb/nanovdb_overrides/README.md.
*/
...
// FVDB FORK: needed for the env-gated tileCount trace in allocateInternalMaskBuffers.
#include <cstdio>
#include <cstdlib>
...
void TopologyBuilder<BuildT>::allocateInternalMaskBuffers(cudaStream_t stream)
{
    ...
    // FVDB DEBUG: trace tile counts to diagnose topology blowup.
    if (std::getenv("FVDB_NANOVDB_TRACE_ALLOCS")) {
        std::fprintf(stderr,
                     "[fvdb/nanovdb] TopologyBuilder.allocateInternalMaskBuffers tileCount=%u upper=%.3fMB lower=%.3fMB\n",
                     hostProcessedRoot()->tileCount(),
                     double(upperSize)/1e6, double(lowerSize)/1e6);
    }
