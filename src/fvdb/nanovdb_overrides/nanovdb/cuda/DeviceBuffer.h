...
/*!
    ...
    \note ================================================================
          ...
          DeviceBuffer.h. The only functional change is that device-side
          allocations go through PyTorch's CUDA caching allocator
          (c10::cuda::CUDACachingAllocator) instead of cudaMallocAsync /
          cudaFreeAsync. This keeps all transient scratch allocations made
          by nanoVDB internals (MergeGrids, TopologyBuilder, DilateGrid,
          etc.) inside the same pool that fvdb / PyTorch tensors use,
          which prevents the allocator from fragmenting into two
          independent pools and avoids OOMs where one pool holds memory
          the other cannot see.

          If you need to resync with upstream, diff against
          build/.../_deps/nanovdb-src/nanovdb/nanovdb/cuda/DeviceBuffer.h
          and port the delta -- only the init() / deviceUpload() / clear()
          / operator= sites should differ.
          ================================================================
*/
...
inline DeviceBuffer& DeviceBuffer::operator=(DeviceBuffer&& other) noexcept
{
    if (mManaged) {// first free all the managed data buffers
        ...
        // FVDB FORK: return device memory to torch's caching allocator instead of cudaFreeAsync.
        for (int i=0; i<mDeviceCount; ++i) {
            if (mGpuData[i] != nullptr) {
                c10::cuda::CUDAGuard deviceGuard(i);
                c10::cuda::CUDACachingAllocator::raw_delete(mGpuData[i]);
            }
        }
    }
    delete [] mGpuData;
...
inline void DeviceBuffer::init(uint64_t size, int device, cudaStream_t stream)
{
    ...
    if (device == cudaCpuDeviceId) {
        ...
        // FVDB FORK: use PyTorch's caching allocator so scratch allocations share
        // a pool with tensor memory. We use `raw_alloc_with_stream` (rather than
        // plain `raw_alloc`) so torch records this stream against the block and
        // defers reuse until work on the stream completes, matching the stream-
        // ordered semantics of the original `cudaMallocAsync(..., stream)` call.
        // Freeing via `raw_delete` is stream-safe.
        if (const char *env = std::getenv("FVDB_NANOVDB_TRACE_ALLOCS")) {
            const size_t cutoff = (env[0] == '2') ? 0 : (1ull << 18); // '2' = trace all, else > 256 KB
            if (size >= cutoff) {
                fprintf(stderr,
                        "[fvdb/nanovdb] DeviceBuffer::init alloc %12zu bytes (%.3f MB) dev=%d\n",
                        size, double(size) / 1e6, device);
            }
        }
        c10::cuda::CUDAGuard deviceGuard(device);
        mGpuData[device] = c10::cuda::CUDACachingAllocator::raw_alloc_with_stream(size, stream);
...
inline void DeviceBuffer::deviceUpload(int device, cudaStream_t stream, bool sync)
{
    ...
    if (mGpuData[device] == nullptr) {
        ...
        // FVDB FORK: use PyTorch's caching allocator. See init() for rationale.
        c10::cuda::CUDAGuard deviceGuard(device);
        mGpuData[device] = c10::cuda::CUDACachingAllocator::raw_alloc_with_stream(mSize, stream);
    }
