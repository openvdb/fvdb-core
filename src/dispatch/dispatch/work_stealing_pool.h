...
private:
    ...
    static Index normalize_grain(Index grain_size, Index n, std::size_t threads_hint) {
        ...
        unsigned_index_t<Index> g = static_cast<unsigned_index_t<Index>>(
            un / static_cast<unsigned_index_t<Index>>(denom));
        if (g == 0) g = 1;
    ...
    struct job final : job_base {
        ...
        job(Index s, Index e, Index g, std::size_t parts, F&& f)
            : job_base(static_cast<std::ptrdiff_t>(std::max<std::size_t>(1, parts)))
        ...
        static void run_impl(job_base* base, std::size_t pid) noexcept {
