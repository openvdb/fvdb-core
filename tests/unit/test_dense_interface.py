...
class TestDenseInterfaceSingle(unittest.TestCase):
    ...
    def test_write_to_dense_cminor_cmajor_dense_grid(self, device, dtype):
        ...
        for eshape in [(3,), (5, 7)]:
            ...
            dense_default = grid.inject_to_dense_cminor(JaggedTensor(sparse_data), min_coord, dense_size).squeeze(0)
            dense_conv = grid.inject_to_dense_cmajor(JaggedTensor(sparse_data), min_coord, dense_size).squeeze(0)
    ...
    def test_read_from_dense_cminor_cmajor_dense_grid(self, device, dtype):
        ...
        for eshape in [(3,), (5, 7)]:
            ...
            sparse_conv = grid.inject_from_dense_cmajor(dense_conv.unsqueeze(0), min_coord)
            sparse_default = grid.inject_from_dense_cminor(dense_default.unsqueeze(0), min_coord)
