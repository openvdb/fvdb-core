...
class TestBasicOps(unittest.TestCase):
    ...
    def test_building_empty_grids_from_ijk(self, device, dtype):
        ...
        with self.assertRaises(ValueError):
            fvdb.gridbatch_from_ijk(grid_ijk_bad)
    ...
    @parameterized.expand(all_device_dtype_combos)
    def test_building_grid_with_one_empty_element_in_jagged_tensor(self, device, dtype):
        shapes = [512, 0, 128]
    ...
    @parameterized.expand(all_device_dtype_combos)
    def test_building_empty_grids_from_points(self, device, dtype):
        batch_size = 1
        grid_ijk = fvdb.JaggedTensor([torch.rand(0, 3) for i in range(batch_size)]).to(device)
        grid = fvdb.gridbatch_from_points(grid_ijk)
        self.assertEqual(len(grid), batch_size)
        self.assertEqual(grid.joffsets[0].item(), 0)
        self.assertEqual(grid.joffsets[1].item(), 0)
        ...
        with self.assertRaises(ValueError):
            fvdb.gridbatch_from_points(grid_ijk_bad)
    ...
    @parameterized.expand(all_device_dtype_combos)
    def test_building_empty_grids_from_nearest_points(self, device, dtype):
        batch_size = 1
        grid_ijk = fvdb.JaggedTensor([torch.rand(0, 3) for i in range(batch_size)]).to(device)
        grid = fvdb.gridbatch_from_nearest_voxels_to_points(grid_ijk)
        self.assertEqual(len(grid), batch_size)
        self.assertEqual(grid.joffsets[0].item(), 0)
        self.assertEqual(grid.joffsets[1].item(), 0)
        ...
        grid_ijk_bad = fvdb.JaggedTensor([torch.rand(0, 2) for i in range(batch_size)]).to(device)
        with self.assertRaises(ValueError):
            fvdb.gridbatch_from_nearest_voxels_to_points(grid_ijk_bad)
    ...
    def test_fvdb_cat(self, device, dtype):
        def _make_random_grid(batch_size):
            ...
            return fvdb.gridbatch_from_ijk(grid_ijk)
    ...
    @parameterized.expand(all_device_dtype_combos)
    def test_fvdb_cat_empty_grid(self, device, dtype):
