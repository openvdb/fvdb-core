...
def _seed_state(device="cuda", dtype=torch.float32, with_features=False, feature_dim: int = 3):
...
def test_grow_disjoint_appends(device):
    ...
    st.weights.copy_(torch.arange(n_before, device=device, dtype=st.weights.dtype) * -1.0)

    new_ijks = torch.tensor([[100, 100, 100], [101, 100, 100]], dtype=torch.int32, device=device)
    st.grow(_make_cpp_ijks(new_ijks))
    n_after = st.active_voxel_count
    assert n_after == n_before + 2, f"disjoint grow should append exactly 2 voxels " f"(got {n_after - n_before})"
    ...
    expected_tsdf_old = torch.arange(n_before, device=device, dtype=st.tsdf.dtype)
    ...
    assert (tsdf_sorted[:3] == 0).all(), "expected 3 zero entries (1 old, 2 newly appended), got " f"{tsdf_sorted[:5]}"
...
def test_grow_overlap_only_preserves_values(device):
    ...
    overlap_ijks = torch.tensor([[0, 0, 0], [1, 1, 1], [3, 3, 3]], dtype=torch.int32, device=device)
    ...
    assert torch.equal(torch.sort(st.tsdf.flatten())[0], torch.sort(tsdf_snapshot.flatten())[0])
    assert torch.equal(torch.sort(st.weights.flatten())[0], torch.sort(weights_snapshot.flatten())[0])
...
def test_features_sidecar_survives_grow(device):
    ...
    n_before = st.active_voxel_count
    st.features.copy_(torch.arange(n_before * 4, device=device, dtype=st.features.dtype).reshape(n_before, 4))
