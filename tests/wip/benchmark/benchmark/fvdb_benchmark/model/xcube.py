...
class ConvBlock(nn.Sequential):
    def __init__(
        self, name: str, in_channels: int, out_channels: int, order: str, backend: str
    ):
        super().__init__()
...
class SparseHead(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "SingleConv",
            ConvBlock(f"{name}_conv_block", in_channels, in_channels, order, backend),
        )
...
class SparseDoubleConv(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "SingleConv1",
            ConvBlock(
                f"{name}_cb1", conv1_in_channels, conv1_out_channels, order, backend
            ),
        )
        self.add_module(
            "SingleConv3",
            ConvBlock(
                f"{name}_cb2", conv1_out_channels, conv1_in_channels, order, backend
            ),
        )
        self.add_module(
            "SingleConv4",
            ConvBlock(
                f"{name}_cb3", conv1_in_channels, conv1_out_channels, order, backend
            ),
        )
        self.add_module(
            "SingleConv2",
            ConvBlock(
                f"{name}_cb4", conv2_in_channels, conv2_out_channels, order, backend
            ),
        )
        ...
        **kwargs,
        ...
        for layer_idx in range(-1, -num_blocks - 1, -1):
            self.struct_convs.add_module(
                f"Struct{layer_idx}",
                SparseHead(
                    f"Struct{layer_idx}", n_features[layer_idx], 2, order, backend
                ),
            )
            if layer_idx < -1:
                ...
                self.upsamplers.add_module(
                    f"Up{layer_idx}",
                    self.wrapper.upsample_generative(f"up-{layer_idx}", 2),
                )
