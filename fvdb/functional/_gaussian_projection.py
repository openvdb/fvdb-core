...
class ProjectedGaussians:
    ...
    Attributes:
        ...
        projection_method: The resolved projection method (never ``AUTO``).
    """
...
def project_gaussians(
    ...
    _OPENCV_MODELS = {
        int(CameraModel.OPENCV_RADTAN_5),
        int(CameraModel.OPENCV_RATIONAL_8),
        int(CameraModel.OPENCV_RADTAN_THIN_PRISM_9),
        int(CameraModel.OPENCV_THIN_PRISM_12),
    }
    cm_int = int(camera_model)
    if cm_int in _OPENCV_MODELS:
        if distortion_coeffs is None:
            raise RuntimeError("distortionCoeffs must be provided for OPENCV_* camera models")
        C = world_to_camera_matrices.size(0)
        if distortion_coeffs.dim() != 2 or distortion_coeffs.size(0) != C or distortion_coeffs.size(1) != 12:
            raise RuntimeError(f"distortionCoeffs must have shape [{C}, 12], got {list(distortion_coeffs.shape)}")
    ...
    if cm_int in _OPENCV_MODELS and resolved == ProjectionMethod.ANALYTIC:
        raise RuntimeError("OPENCV_* camera models require ProjectionMethod::UNSCENTED or AUTO")
